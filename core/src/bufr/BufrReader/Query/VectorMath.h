...
namespace bufr {
    ...
    /// \return Scalar value.
    template<typename T>
    T product(const std::vector<T> &vec) noexcept
    {
    ...
    T product(typename std::vector<T>::const_iterator begin,
              typename std::vector<T>::const_iterator end) noexcept
    {
        return std::accumulate(begin, end, 1, std::multiplies<T>());
    }

    /// \brief Sum all the values in a vector.
    /// \param vec The vector to sum.
    /// \return Scalar value.
    template<typename T>
    T sum(const std::vector<T>& vec) noexcept
    {
        return std::accumulate(vec.begin(), vec.end(), 0);
    }

    template<typename T>
    std::vector<T> linearSpace(T start, T end) noexcept
    {
