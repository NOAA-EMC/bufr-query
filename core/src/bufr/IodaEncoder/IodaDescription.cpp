...
namespace bufr {
    IodaDescription::IodaDescription(const std::string& yamlFile) : outputPathTemplate_("")
    {
      auto conf = eckit::YAMLConfiguration(eckit::PathName(yamlFile));
      init(conf.getSubConfiguration("ioda"));
    }

    IodaDescription::IodaDescription(const eckit::Configuration& conf) : outputPathTemplate_("")
    {
      init(conf);
    }
