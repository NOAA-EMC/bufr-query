...
namespace bufr {
    ...
    std::map<SubCategory, ioda::ObsGroup>
        IodaEncoder::encode(const std::shared_ptr<DataContainer>& dataContainer,
                            const IodaEncoder::Backend& backend,
                            bool append)
    {
        ...
        for (const auto& categories : dataContainer->allSubCategories())
        {
            ...
            switch (backend.backendName)
            {
                case ioda::Engines::BackendNames::ObsStore:
                    break;
                case ioda::Engines::BackendNames::Hdf5File:
                    if (dataContainer->allSubCategories().size() > 1)
                    {
                        backendParams.fileName = makeStrWithSubstitions(
                          description_.getOutputPathTemplate(), substitutions);

                        if (backendParams.fileName.empty())
                        {
                            throw eckit::BadParameter(
                              "You must define \"outputPathTemplate\" in the ioda description to "
                              "write categorized (split) data.");
                        }
                    }
                    else
                    {
                        backendParams.fileName
                          = makeStrWithSubstitions(backend.backendPath, substitutions);

                        if (backendParams.fileName.empty())
                        {
                            backendParams.fileName =
                              makeStrWithSubstitions(description_.getOutputPathTemplate(),
                                                     substitutions);
                        }
                    }
                    break;
                default:
                    throw eckit::BadParameter( "IodaEncoder encountered unknown backend type.");
            }
            ...
            auto rootGroup = ioda::Engines::constructBackend(ioda::Engines::BackendNames::ObsStore,
                                                             backendParams);
    ...
    bool IodaEncoder::existsInNamedPath(const Query& path, const NamedPathDims& pathMap) const
    {
    ...
    DimensionDescription IodaEncoder::dimForDimPath(const Query& path,
