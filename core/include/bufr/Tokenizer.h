...
namespace bufr {
  ...
  public:
    ...
    size_t index() const { return std::stoi(std::string(++str_.begin(), --str_.end())); }
  };
  ...
  public:
    ...
    /// \brief Tokenize the str_ into tokens.
    void tokenize() final {
      static const auto filterRegex = std::regex("\\{(\\d+(\\-\\d+)?)(,\\d+(\\-\\d+)?)*\\}");
  ...
  public:
    constexpr static const char* Pattern  = "([A-Z0-9_\\*\\/]+((\\[\\d+\\])?)+(\\{[0-9\\-,]+\\})?)+";
  ...
  public:
    ...
    void tokenize() final {
      ...
      static const auto queryRegex = std::regex("((?=\\,)|())" + std::string(QueryToken::Pattern));
    ...
    std::vector<std::shared_ptr<QueryToken>> queryTokens() const final { return queries_; }
