    ...
    class IodaEncoder
    {
     public:
        struct Backend
        {
          ...
          Backend() : backendName(ioda::Engines::BackendNames::ObsStore),
                      backendPath("") {};

          Backend(const ioda::Engines::BackendNames& backendName, const std::string& backendPath) :
            backendName(backendName), backendPath(backendPath) {};
        };
