...
namespace bufr {
  ...
  struct BufrNode : public std::enable_shared_from_this<BufrNode> {
    ...
    bool isQueryPathParentNode() {
      return type == Typ::DelayedRep || type == Typ::FixedRep || type == Typ::DelayedRepStacked
    ...
    bool isLeaf() const { return type == Typ::Number || type == Typ::Character; }

    /// \brief Does this type of node contain children?
    /// \return True if this node is a container node.
    bool isContainer() const {
      return type == Typ::DelayedRep || type == Typ::DelayedRepStacked || type == Typ::DelayedBinary
             || type == Typ::FixedRep || type == Typ::Repeat || type == Typ::StackedRepeat
             || type == Typ::Sequence || type == Typ::Subset;
    }
    ...
    /// \brief Is this node a stacked repeat node (surrounded by [ ] in BUFR file)?
    /// \return True if this node is a stacked repeat node.
    bool isStack() const {
      return type == Typ::DelayedRepStacked || type == Typ::StackedRepeat;
    }
  ...
  typedef std::vector<std::shared_ptr<bufr::BufrNode>> BufrNodeVector;
  ...
  private:
    ...
    /// \brief Parses the BUFR message subset Meta data tables in a recursive fashion.
